...
/**
 * Notion public site responses can wrap records as { value: { value, role } }.
 * Normalize them to the shape expected by notion-utils / NotionNext.
 */
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const normalizedRecordMap = normalizeRecordMap(pageChunk?.recordMap)
    const end = new Date().getTime()
    console.log(
